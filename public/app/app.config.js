...
(function () {
  ...
  configHttp.$inject = ['$httpProvider', '$locationProvider', '$sceDelegateProvider'];

  function configHttp($httpProvider, $locationProvider, $sceDelegateProvider) {
    // Hashbang routing (no server rewrite needed → deploys anywhere, incl. Vercel static)
    $locationProvider.hashPrefix('!');

    // Allow the Google Maps embed iframe (ng-src) — Angular blocks external
    // resource URLs by default (whitelist is 'self' only), which would stop
    // the map on the Home & Contact pages from loading.
