...
(function () {
  ...
  function HomeController($interval, $scope, ConfigService, ApiService, SeoService) {
    ...
    // ---- Hero slider ----
    vm.slides = [
      { img: 'assets/img/hero/hero-1.svg', title: 'Nurturing Faith,<br>Knowledge &amp; Character', text: 'A modern Islamic school where every child is inspired to learn, lead and grow with strong values.' },
    ...
    vm.stats = (vm.cfg.stats && vm.cfg.stats.length) ? vm.cfg.stats : [
      { label: 'Students', value: 300, suffix: '+', icon: 'bi-people' },
      { label: 'Teachers', value: 12, suffix: '+', icon: 'bi-person-workspace' },
      { label: 'Years of Excellence', value: 10, suffix: '+', icon: 'bi-award' },
      { label: 'Pass Percentage', value: 90, suffix: '%', icon: 'bi-mortarboard' }
    ];
    ...
    vm.testimonials = [
      ...
      { name: 'Abdul Hameed Maniyar', role: 'Alumnus', text: 'Alfalah gave me the foundation and confidence for my engineering journey.' },
      { name: 'Dilawar Chouhan', role: 'Parent', text: 'Smart classes and regular updates keep us involved in our son\'s progress.' }
    ];
